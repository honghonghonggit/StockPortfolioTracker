...
public class GoalManagementPanel extends JPanel {
    ...
    private Map<String, Double> goalMap;
    ...
    private void loadData() {
        try {
            ...
            stockList = !loaded.isEmpty() ? loaded : createDummyData();
        } catch (IOException e) {
    ...
    private JPanel createGoalTablePanel() {
        ...
        tableModel.addTableModelListener(e -> {
            if (e.getColumn() == 2 && e.getType() == javax.swing.event.TableModelEvent.UPDATE) {
                ...
                if (row >= 0 && row < stockList.size()) handleGoalEdit(row);
            }
        });
    ...
    private void refreshTable() {
        ...
        for (Stock s : stockList) {
            ...
            tableModel.addRow(new Object[]{s.getName(), currentStr, targetStr, achievedStr, gapStr});
        }
    }

    private void updateSummaryCards() {
        if (achievedCountLabel != null) achievedCountLabel.setText(countAchieved() + "개");
        if (notAchievedCountLabel != null) notAchievedCountLabel.setText(countNotAchieved() + "개");
        if (avgGapLabel != null) avgGapLabel.setText(rateInputFormat.format(calcAvgGap()) + " %p");
    }

    private long countAchieved() {
        return stockList.stream().filter(s -> {
    ...
    private double calcAvgGap() {
        ...
        for (Stock s : stockList) {
            Double target = goalMap.get(s.getName());
            if (target != null) gaps.add(target - s.getProfitRate());
        }
    ...
    private void styleTable(JTable table) {
        ...
        editorField.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(ACCENT_BLUE, 2), new EmptyBorder(4, 8, 4, 8)));
    ...
    private DefaultTableCellRenderer createGoalCellRenderer() {
        return new DefaultTableCellRenderer() {
            @Override public Component getTableCellRendererComponent(JTable tbl, Object value,
                ...
                setBackground(isSelected ? new Color(60, 60, 90) : new Color(50, 50, 72));
                return this;
            }
        };
    }
        ...
        return new DefaultTableCellRenderer() {
            @Override public Component getTableCellRendererComponent(JTable tbl, Object value,
                ...
                if (v.contains("초과달성")) {
                    setForeground(ACCENT_GREEN);
                    setFont(new Font("맑은 고딕", Font.BOLD, 13));
                } else if (v.contains("남음")) { setForeground(ACCENT_AMBER); }
